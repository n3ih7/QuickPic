...
if not os.path.exists(database_file):
    ...
    with urllib.request.urlopen('https://www.cse.unsw.edu.au/~cs2041/18s2/activities/instacram/test.sqlite3',
                                context=ssl.SSLContext(ssl.PROTOCOL_TLSv1)) as response:
        db = response.read()
...
try:
    ...
    if sys.version_info < (3, 6):
        print(
            'The backend requires Python 3.6 or later - you appear to be using Python {}.{}'.format(*sys.version_info),
            file=sys.stderr)
    else:
